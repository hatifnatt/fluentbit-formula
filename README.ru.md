...
**Важно**: Изменения в systemd drop-in конфигурации (включая включение/отключение `fluentbit:service:reload`) всегда вызывают полный перезапуск сервиса, независимо от настройки `fluentbit:service:reload`. Это гарантирует, что все изменения аргументов запуска будут применены.
Только изменения в файлах конфигурации Fluent Bit (не в systemd конфигурации) будут использовать перезагрузку вместо перезапуска при включенном `fluentbit:service:reload: true`.
